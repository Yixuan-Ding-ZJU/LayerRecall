...
class WanVAE_(nn.Module):
    ...
    def cached_decode(self, z, scale):
        if not hasattr(self, "_decode_cached_started"):
            self.clear_cache()
        if isinstance(scale[0], torch.Tensor):
            z = z / scale[1].view(1, self.z_dim, 1, 1, 1) + scale[0].view(
                1, self.z_dim, 1, 1, 1)
        ...
        for i in range(iter_):
            self._conv_idx = [0]
            first_chunk = not self._decode_cached_started
            outs.append(
                self.decoder(
                    x[:, :, i:i + 1, :, :],
                    feat_cache=self._feat_map,
            ...
            self._decode_cached_started = True
    ...
    def clear_cache(self):
        ...
        self._decode_cached_started = False
